...
#Add or remove targets as required so that only those rules are executed
...
rule align:
	...
	params:
		hg19 = "/n/scratch3/users/s/sz166/cfmedip/ref_files/indexes/hg19"		
	shell:
		"bowtie2 -x {params.hg19} -p 4 -S {output.bam} -1 {input.fq1} -2 {input.fq2} 2> {output.log}"
...
# identify differentially methylated regions in a reference panel
#reference_DMRS.bed file has all the differentially methylated regions with p value <0.05. Use this file to apply more stringer logFC or p value cut offs for the remaining steps
rule reference_DMRs:
	input:
